...
from utils.rotationTransform3 import rotationTransform3

class DizzyRNNCellOpt3(tf.nn.rnn_cell.RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
        ...
        with vs.variable_scope(scope or type(self).__name__):
            ...
            if self._bottom == True:
                [state_out] = rotationTransform3([t_state], self._num_units, scope)
                input_out = linearTransformWithBias([inputs],
                    self._num_units, bias=False, scope=scope)
            else:
                [state_out, input_out] = \
                    rotationTransform3([t_state, t_inputs],
