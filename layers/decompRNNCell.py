...
class DecompRNNCell(tf.nn.rnn_cell.RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
        with vs.variable_scope(scope or type(self).__name__):
            ...
            state_scale, state_sigma = diagonalTransform(state_rot, self._num_units)
            sigmas.append(state_sigma)
            ...
            if self._bottom:
                ...
                input_rot = rotationTransform(tf.transpose(inputs), self._num_units, self._num_params,
                    self._cos_list,  self._sin_list, self._nsin_list,
                    self._cos_idxs, self._sin_idxs, self._nsin_idxs)

                input_scale, input_sigma = diagonalTransform(input_rot, self._num_units)
                sigmas.append(input_sigma)

                input_out = rotationTransform(input_scale, self._num_units, self._num_params,
                    self._cos_list,  self._sin_list, self._nsin_list,
                    self._cos_idxs, self._sin_idxs, self._nsin_idxs)
